...
class Mining extends BaseController
{
    ...
    public function asosiasi($dataItemset2 = [], $dataItemset3 = [])
    {
        ...
        $data['keterangan'] = ['tanggalMulai' => $this->tanggalMulai, 'tanggalSelesai' => $this->tanggalSampai, 'jumlahTransaksi' => $this->jumlahTransaksi, 'support' => $this->support, 'confiden' => $this->confiden];
        ...
        if (!empty($data['saran'])) {
            foreach ($data['saran'] as $key => $v) {
                array_push($save, [
                    ...
                    'tgl_mulai' => $data['keterangan']['tanggalMulai'],
                    'tgl_selesai' => $data['keterangan']['tanggalSelesai'],
                ]);
            }
            $this->save_riwayat($save);
        }
